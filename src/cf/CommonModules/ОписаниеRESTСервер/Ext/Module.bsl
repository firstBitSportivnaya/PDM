...
Функция ПолучитьОписаниеРесурса(ОписаниеСервиса, ВызываемыйРесурс) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		СоставURLЗапроса = ОбщегоНазначенияRESTСервер.РазделитьURL(ВызываемыйРесурс);
		
		Для Каждого КЗ Из ОписаниеСервиса.Ресурсы Цикл
			
			ПутьРесурса = КЗ.Ключ;
			
			Если ОбщегоНазначенияRESTСервер.ЭтоСлужебныйРесурс(ПутьРесурса) Тогда
				Продолжить;
			КонецЕсли;
			...
			СоставURLРесурса = ОбщегоНазначенияRESTСервер.РазделитьURL(ПутьРесурса);
...
Функция ПолучитьПользовательскиеНастройки()
	...
	|	СервисыREST.ИспользуютсяПользовательскиеФункции КАК ИспользуютсяПользовательскиеФункции
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА МетодыРесурсовREST.Владелец.Владелец.ИспользуютсяПользовательскиеФункции
	|			ТОГДА МетодыРесурсовREST.ОбработчикПользовательскаяФункция
	|		ИНАЧЕ
	|			МетодыРесурсовREST.Обработчик
	|	КОНЕЦ КАК Обработчик,
...
Процедура ЗаполнитьОписаниеОпределенийОбъектов(ОписаниеСервиса, ПользовательскиеНастройки)
	...
	ЗаполнитьОписаниеОпределенийОбъектовРекурсивно(ОписаниеСервиса, ОписаниеСервиса.ОпределенияОбъектов, ПользовательскиеНастройки);
...
Процедура ЗаполнитьОписаниеОпределенийОбъектовРекурсивно(ОписаниеСервиса, ОпределенияОбъектов, ПользовательскиеНастройки)
	...
	Для Каждого КЗ Из ОпределенияОбъектов Цикл
		...
		Для Каждого КЗ Из ОписаниеОпределенияОбъекта.СвойстваОбъекта Цикл
			...
			ИначеЕсли ОписаниеСвойстваОбъекта.ТипДанных = "Объект" Тогда
				
				ДочернееОпределение = Новый Соответствие;
				ДочернееОпределение.Вставить(ОписаниеСвойстваОбъекта.ОпределениеОбъекта);
				ЗаполнитьОписаниеОпределенийОбъектовРекурсивно(ОписаниеСервиса, ДочернееОпределение, ПользовательскиеНастройки);
...
Процедура ДобавитьВОписаниеСервисаМетодДокументации(ОписаниеСервиса)
	...
	ОписаниеМетода.Обработчик = "ОбработкаВызоваRESTСервер.ОбработчикМетодаДокументацииСервиса";
...
Процедура ДобавитьВОписаниеСервисаМетодИконка16Документации(ОписаниеСервиса)
	...
	ОписаниеМетода = СтруктураДанныхМетодЗапроса();
	ОписаниеМетода.МетодЗапроса = Перечисления.МетодыЗапросаREST.GET;
	ОписаниеМетода.Обработчик = "ОбработкаВызоваRESTСервер.ОбработчикМетодаИконка16ДокументацииСервиса";
	ОписаниеМетода.Ответы.Вставить(Перечисления.ТипыОтветовREST.Успешно, ОписаниеОтвета);
...
Процедура ДобавитьВОписаниеСервисаМетодСтилейДокументации(ОписаниеСервиса)
	...
	ОписаниеМетода = СтруктураДанныхМетодЗапроса();
	ОписаниеМетода.МетодЗапроса = Перечисления.МетодыЗапросаREST.GET;
	ОписаниеМетода.Обработчик = "ОбработкаВызоваRESTСервер.ОбработчикМетодаСтилейДокументацииСервиса";
	ОписаниеМетода.Ответы.Вставить(Перечисления.ТипыОтветовREST.Успешно, ОписаниеОтвета);
...
Процедура ДобавитьВОписаниеСервисаМетодСкриптовДокументации(ОписаниеСервиса)
	...
	ОписаниеМетода = СтруктураДанныхМетодЗапроса();
	ОписаниеМетода.МетодЗапроса = Перечисления.МетодыЗапросаREST.GET;
	ОписаниеМетода.Обработчик = "ОбработкаВызоваRESTСервер.ОбработчикМетодаСкриптовДокументацииСервиса";
	ОписаниеМетода.Ответы.Вставить(Перечисления.ТипыОтветовREST.Успешно, ОписаниеОтвета);
...
Процедура ДобавитьВОписаниеСервисаМетодСпецификации(ОписаниеСервиса)
	...
	ОписаниеМетода = СтруктураДанныхМетодЗапроса();
	ОписаниеМетода.МетодЗапроса = Перечисления.МетодыЗапросаREST.GET;
	ОписаниеМетода.Обработчик = "ОбработкаВызоваRESTСервер.ОбработчикМетодаСпецификацииСервиса";
	ОписаниеМетода.Ответы.Вставить(Перечисления.ТипыОтветовREST.Успешно, ОписаниеОтвета);
...
Функция СтруктураДанныхСервис()
	...
	Результат.Вставить("ИспользуютсяПользовательскиеФункции", Ложь);
