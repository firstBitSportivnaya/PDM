...
Функция ПолучитьЗначение(Имя) Экспорт
	...
	Если ПланыВидовХарактеристик.ПредопределенныеЗначения[Имя].СписокЗначений Тогда
		Значение = ОбщегоНазначения.ВыгрузитьКолонку(
			ПланыВидовХарактеристик.ПредопределенныеЗначения[Имя].ЗначенияЭлементов, "Значение", Истина);
	ИначеЕсли ПланыВидовХарактеристик.ПредопределенныеЗначения[Имя].Пароль Тогда
		Значение = ПолучитьПарольПоИмени(Имя);
	Иначе
		Значение = ПланыВидовХарактеристик.ПредопределенныеЗначения[Имя].Значение;
	КонецЕсли;
...
Функция ПолучитьЗначения(Имена, ВРазрезеКлючей = Ложь) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|ИЗ
		|	ПланВидовХарактеристик.ПредопределенныеЗначения КАК ПредопределенныеЗначения
		|ГДЕ
		...
		|	ПредопределенныеЗначения1.Значение КАК Значение,
		|	ПУСТАЯТАБЛИЦА.( КАК Значение) КАК ЗначенияЭлементов,
		|	ПредопределенныеЗначения1.Пароль КАК Пароль,
		|	ЛОЖЬ КАК СписокЗначений
		|ИЗ
		|	ПредопределенныеЗначения КАК ПредопределенныеЗначения
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ПредопределенныеЗначения КАК ПредопределенныеЗначения1
		|		ПО ПредопределенныеЗначения.ПредопределенноеЗначение = ПредопределенныеЗначения1.Ссылка
		|			И (НЕ ПредопределенныеЗначения1.СписокЗначений)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	ПредопределенныеЗначения1.ЗначенияЭлементов.(
		|		Значение
		|	),
		|	ЛОЖЬ,
		|	ИСТИНА
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ПредопределенныеЗначения КАК ПредопределенныеЗначения1
		|		ПО ПредопределенныеЗначения.ПредопределенноеЗначение = ПредопределенныеЗначения1.Ссылка
		|			И (ПредопределенныеЗначения1.СписокЗначений)";
...
Функция ПолучитьПарольПоИмени(Имя)
	...
	ДанныеХранилища = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(
			Строка(ПланыВидовХарактеристик.ПредопределенныеЗначения[Имя].УникальныйИдентификатор()));
