...
Функция ОбработатьЗапрос(Запрос) Экспорт
	...
	// Начать замер
	КонтекстЗамера = ЛогированиеRESTСервер.ИнициализироватьЗамер(КонтекстЗапроса);
	...
	Если Не АвторизацияПройдена(КонтекстЗапроса) Тогда
		...
		ЛогированиеRESTСервер.ЗавершитьЗамер(КонтекстЗамера, КонтекстОтвета);
	...
	Попытка
		Если Не КонтекстЗапроса.ЭтоСлужебныйРесурс И КонтекстЗапроса.ОписаниеСервиса.ИспользуютсяПользовательскиеФункции Тогда
			РезультатОбработчика = ПолучитьРезультатОбработкиИзПользовательскойФункции(КонтекстЗапроса, КонтекстЗамера);
		Иначе
	...
	Исключение
		...
		ЛогированиеRESTСервер.ЗафиксироватьОшибку(КонтекстЗамера, "Ошибка обработчика метода", ОписаниеОшибки, ИнформацияОбОшибке);
		ЛогированиеRESTСервер.ЗавершитьЗамер(КонтекстЗамера, КонтекстОтвета);
	...
	Исключение
		...
		ЛогированиеRESTСервер.ЗафиксироватьОшибку(КонтекстЗамера, "Ошибка формирования метода", ОписаниеОшибки, ИнформацияОбОшибке);
		ЛогированиеRESTСервер.ЗавершитьЗамер(КонтекстЗамера, КонтекстОтвета);
...
Функция ОбработчикМетодаДокументацииСервиса(КонтекстЗапроса, КонтекстЗамера) Экспорт
	...
	Если ЗначениеЗаполнено(КонтекстЗапроса.ИсходныйЗапрос.Порт) Тогда
		URL = Лев(КонтекстЗапроса.КорневойURL, СтрНайти(КонтекстЗапроса.КорневойURL, "/",,,3) - 1)
			+ ":" + Формат(КонтекстЗапроса.ИсходныйЗапрос.Порт, "ЧГ=0")
			+ Прав(КонтекстЗапроса.КорневойURL, СтрНайти(КонтекстЗапроса.КорневойURL, "/",,,3) + 2);
	Иначе
		URL = КонтекстЗапроса.КорневойURL;
	КонецЕсли;
	
	Иконка16Ссылка = URL + "/favicon-16x16.png";
	Иконка32Ссылка = URL + "/favicon-32x32.png";
	СтилиСсылка = URL + "/swagger-ui.css";
	СкриптыСсылка = URL + "/swagger-ui-bundle.js";
	СпецификацияСсылка = URL + "/openapi.json";
...
Функция ОбработчикМетодаСпецификацииСервиса(КонтекстЗапроса, КонтекстЗамера) Экспорт
	...
	СодержаниеОтвета = СпецификацияRESTСервер.ПолучитьСпецификациюСервиса(КонтекстЗапроса.ОписаниеСервиса, КонтекстЗапроса.КорневойURL);
...
Процедура ОпределитьОписаниеЗапроса(КонтекстЗапроса)
	...
	// ОписаниеРесурса
	ОписаниеРесурса = ОписаниеRESTСервер.ПолучитьОписаниеРесурса(ОписаниеСервиса, КонтекстЗапроса.ВызываемыйРесурс);
...
Функция ПроверитьКонтекстЗапросаПоСпецификации(КонтекстЗапроса)
	...
	Если ОписаниеРесурса.ЕстьПараметрыURL Тогда
		...
		СоставURLЗапроса = ОбщегоНазначенияRESTСервер.РазделитьURL(КонтекстЗапроса.ВызываемыйРесурс);
	...
	Для Каждого КЗ Из ОписаниеМетода.Параметры Цикл
		...
		Если ТипДанных = "Перечисление" Тогда
			...
			Попытка
				ПриведенноеЗначение = СпецификацияRESTСервер.ПолучитьСистемныйОбъектПоОписанию(ТипДанных, ОпределениеОбъекта, Значение, ОпределенияОбъектов);
			Исключение
	...
	Если Не ТипДанных = "Пусто" Тогда
		...
		Если ТипДанных = "Объект" Или ТипДанных = "Массив" Тогда
			...
			Попытка
				СистемныйОбъект = СпецификацияRESTСервер.ПолучитьСистемныйОбъектПоОписанию(ТипДанных, ОпределениеОбъекта, ПриведенноеЗначение, ОпределенияОбъектов);
			Исключение
...
Функция ПолучитьРезультатОбработкиИзПользовательскойФункции(КонтекстЗапроса, КонтекстЗамера)
	...
	НовыйПараметр.Параметр = "КонтекстЗапроса";
	НовыйПараметр.Значение = КонтекстЗапроса;
	
	НовыйПараметр = ТаблицаПараметров.Добавить();
	НовыйПараметр.Параметр = "КонтекстЗамера";
	НовыйПараметр.Значение = КонтекстЗамера;
	
	РезультатОбработки = ИнтеграцияСБотамиСлужебныйСервер.ВыполнитьПользовательскуюФункцию(
		КонтекстЗапроса.ОписаниеМетода.Обработчик, ТаблицаПараметров);
...
Функция СформироватьДанныеКонтекстаЗапроса(Запрос)
	...
	ЭтоСлужебныйРесурс = ОбщегоНазначенияRESTСервер.ЭтоСлужебныйРесурс(ВызываемыйРесурс);
...
Функция СформироватьДанныеИсходногоЗапроса(Запрос)
	...
	Если ЗначениеЗаполнено(Результат.Заголовки.Получить("Host")) Тогда
		СерверИПорт = СтрРазделить(Результат.Заголовки.Получить("Host"), ":");
		Если СерверИПорт.Количество() > 1 Тогда
			Результат.Порт = Число(СерверИПорт[1]);
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьДанныеКонтекстаОтвета(РезультатОбработчика, КонтекстЗапроса)
	...
	Если ТипОтвета = Перечисления.ТипыОтветовREST.Успешно Тогда
		...
		ИначеЕсли ТипДанных <> "Пусто" Тогда
			...
			Объект = СпецификацияRESTСервер.ПолучитьОбъектПоОписанию(ТипДанных, ОпределениеОбъекта, СодержаниеОтвета, ОпределенияОбъектов);
