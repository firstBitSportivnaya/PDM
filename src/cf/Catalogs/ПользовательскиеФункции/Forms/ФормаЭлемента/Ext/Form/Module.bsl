...
Процедура ЗадатьПараметры(Команда)
	...
	ОткрытьФорму("Справочник.ПользовательскиеФункции.Форма.ФормаПараметров", ПараметрыФормы, ЭтотОбъект,,,,
...
#Если ВебКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Отладка не предусмотрена в веб-клиенте.'");
#Иначе
...
Процедура УправлениеЭлементамиФормы()
	...
	флКонтекстЗагрузкаЭксель = ?(Объект.Контекст = Перечисления.КонтекстыВыполненияПользовательскихФункций.ЗагрузкаЭксель,
		Истина, Ложь);
...
Процедура ДобавитьПараметры()
	...
	Справочники.ПользовательскиеФункции.ЗаполнитьПараметры(Объект);
	ПолучитьНазваниеПодпрограммыСПараметрамиСервер();
...
Процедура ПолучитьНазваниеПодпрограммыСПараметрамиСервер()
	...
	СтруктураЗаголовка = Справочники.ПользовательскиеФункции.ПолучитьНазваниеПодпрограммыСПараметрами(Объект);
...
Процедура УпаковатьТаблицу(ИмяТаблицы, СтрТаблиц)
	...
	СтрТаблиц.Вставить(ИмяТаблицы, ОбщегоНазначенияУПОКлиентСервер.УпаковатьДанныеФормыКоллекция(
...
Процедура ВыполнитьКодСервер(ЗначенияПараметров)
	...
	Ном = 1;
	ЗначенияПараметровКопия = ОбщегоНазначенияКлиентСервер.СкопироватьСоответствие(ЗначенияПараметров);
	Для Каждого КлючЗначение Из ЗначенияПараметровКопия Цикл
		НоваяСтрока = ТабПараметров.Добавить();
		НоваяСтрока.Номер = Ном;
		НоваяСтрока.Параметр = КлючЗначение.Ключ;
		Если СтрНачинаетсяС(КлючЗначение.Значение, "e1cib/tempstorage/") > 0 Тогда
			НоваяСтрока.Значение = ПолучитьЗначениеИзХранилищаРекурсивно(КлючЗначение.Значение);
		Иначе
	...
	Попытка
		...
		Результат = ОбщегоНазначенияУПОСервер.ВыполнитьПользовательскуюФункциюСПараметрами(ТекстФункции, 
...
Процедура ОткрытьФормуВыбораПоля()
	...
	ОткрытьФорму("ОбщаяФорма.ФормаВыбораПоля", ПараметрыФормы, ЭтотОбъект,,,,
...
Процедура ОткрытьКонструкторУсловногоВыражения()
	...
	ОткрытьФорму("Справочник.ПользовательскиеФункции.Форма.УсловныйОператор", ПараметрыФормы, ЭтотОбъект,,,,
...
#Если ВебКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Извлечение исходников не предназначено для веб-клиента'");
#Иначе	
	Попытка
